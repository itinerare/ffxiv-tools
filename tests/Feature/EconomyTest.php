...
class EconomyTest extends TestCase {
    ...
    public function testGetCraftingCalc($world, $job, $withCookie, $expected, $status): void {
        ...
        if ($expected && $job) {
            ...
            $response->assertSee('Showing '.config('ffxiv.economy.crafting.jobs')[$job].' results for '.ucfirst($world));
    ...
    public function testGetDiadem($world, $initialized, $withCookie, $expected, $status): void {
        ...
        if ($initialized) {
            $items = collect(config('ffxiv.economy.diadem_items.node_data'))->flatten();
